    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0}>
        <div
          ...
          className={cn(
            "group/sidebar-wrapper flex min-h-svh h-full w-full has-data-[variant=inset]:bg-background dark:has-data-[variant=inset]:bg-background-subtle",
            className,
          )}
          ref={ref}
          {...props}
  ...
  return (
    ...
    >
      ...
      >
        <div
          data-sidebar="sidebar"
          className="fixed top-0 bottom-0 border-r-[0.5px] isolate z-30 flex h-full w-(--sidebar-width) flex-col bg-background-subtle dark:bg-background group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border-[0.5px] group-data-[variant=floating]:border-border-element group-data-[variant=floating]:shadow-sm"
        >
          {children}
        </div>
      </div>
  ...
  ({ className, ...props }, ref) => {
    ...
    return (
      <button
        ...
        className={cn(
          "absolute inset-y-0 z-40 hidden w-4 -translate-x-1/2 transition-all ease-linear after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] hover:after:bg-border-element group-data-[side=left]:-right-4 group-data-[side=right]:left-0 sm:flex",
          "in-data-[side=left]:cursor-w-resize in-data-[side=right]:cursor-e-resize",
          "[[data-side=left][data-state=collapsed]_&]:cursor-e-resize [[data-side=right][data-state=collapsed]_&]:cursor-w-resize",
          "group-data-[collapsible=offcanvas]:translate-x-0 group-data-[collapsible=offcanvas]:after:left-full hover:group-data-[collapsible=offcanvas]:bg-background-element",
...
const SidebarInset = React.forwardRef<HTMLDivElement, React.ComponentProps<"main">>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      className={cn(
        "relative flex min-h-svh min-w-0 h-full flex-1 flex-col bg-background-subtle dark:bg-background",
  ...
  ({ className, asChild = false, ...props }, ref) => {
    ...
    return (
      <Comp
        ...
        className={cn(
          "duration-200 flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium text-foreground-subtle outline-hidden ring-info-focus-ring transition-[margin,opa] ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
...
const SidebarMenu = React.forwardRef<HTMLUListElement, React.ComponentProps<"ul">>(({ className, ...props }, ref) => (
  <ul
    ref={ref}
    data-sidebar="menu"
    className={cn("flex w-full min-w-0 flex-col gap-1 mx-0 my-0 list-none [&>li]:mt-0 [&>li]:pl-0", className)}
    {...props}
  />
));
...
const sidebarMenuButtonVariants = cva({
  base: "peer/menu-button text-foreground-subtle flex w-full items-center gap-2 overflow-hidden rounded-sm p-2 text-left text-sm outline-hidden ring-info-focus-ring transition-[width,height,padding] focus-visible:ring-2 data-[active=true]:bg-background dark:data-[active=true]:bg-background-subtle data-[active=true]:font-medium data-[active=true]:text-foreground active:bg-background-element-active active:text-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[state=open]:bg-background-element-selected data-[state=open]:text-foreground data-[state=open]:hover:text-foreground group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
  variants: {
    variant: {
      default:
    ...
    size: {
      ...
      sm: "h-[28px] text-xs",
      lg: "h-[36px] text-sm group-data-[collapsible=icon]:p-0! rounded-md",
    },
  },
  defaultVariants: {
    variant: "default",
