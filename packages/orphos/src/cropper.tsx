...
function Cropper({
  ...
  return (
    <CropperPrimitive.Root
      data-slot="cropper"
      className={cn(
        "relative flex w-full cursor-move touch-none items-center justify-center overflow-auto focus:outline-none",
        className
      )}
      {...props}
    />
...
function CropperCropArea({
  ...
  return (
    <CropperPrimitive.CropArea
      ...
      className={cn(
        "pointer-events-none absolute border-3 border-white shadow-[0_0_0_9999px_rgba(0,0,0,0.75)] in-[[data-slot=cropper]:focus-visible]:ring-[3px] in-[[data-slot=cropper]:focus-visible]:ring-focus-ring",
        className
      )}
      {...props}
    />
